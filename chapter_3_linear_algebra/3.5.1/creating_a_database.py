...
#call the getenburg api and get a dictionary with keys authors and values arrays of words with no-special-charachters and delimeted by ' '
    ...
    for book in response['results']:
        try:
            try:
                ...
                try:
                    book_text = requests.get(book.get('formats', {}).get('text/plain; charset=utf-8', '')).text.lower()
                except: 
                    print('No cigar: ', book['title'])
            book_text = "".join(c if c.isalpha() and c!= '***' else " " for c in book_text)
            words[book.get('authors', [{}])[0].get('name', '')] = book_text[book_text.find('***') + 170:char_num].split(' ')
        except:
            print('No cigar: ', book.get('title', ''))
...
def get_raw_words_by_book(stem_num):
    ...
    for book, words in book_data.items():
        print('Author added: ', book)
        for word in word_set:
            df.loc[word, book] = words.count(word)
    df = df.fillna(0)
    #drop all the empty strings that got counted
